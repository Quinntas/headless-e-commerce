...
export default function CartModal({modalStateChange, modalState}:
    ...
    return <>
        <div
            ...
            className={`bg-black z-40 inset-0 w-screen ${modalState ? "" : "hidden"} h-screen fixed opacity-30 transition duration-300`}>
        </div>
        <div
            className={`opacity-100 bg-white fixed right-0 top-0 z-50 w-[400px] h-full border ${modalState ? "" : "translate-x-full"} shadow-2xl duration-300 ease-in-out transition-transform`}>
            <div className="h-full w-full flex text-black flex-col">
                <div className="flex justify-between w-full pr-4 pl-4 pt-5">
                    <h1 className="font-bold text-xl md:text-2xl m-0 text-heading">Carrinho de Compras</h1>
                    <button type="button" onClick={modalStateChange}>
                        <IoClose color="black" size="20" className={"text-black mt-1 md:mt-0.5"}/>
                    </button>
                </div>
                <div className={"w-full h-full flex flex-col justify-between items-center pt-6"}>
                    <ScrollBar className={"cart-scrollbar w-full flex flex-grow"}>
                            ...
                            <div className="flex items-center pr-4 pl-4" key={product.id}>
                                ...
                                <div className="flex items-center justify-between w-full">
                                    <div className="flex flex-col justify-between pl-3">
                                        <Link href={""}
                                              className="truncate text-sm text-heading mb-1.5 -mt-">{product.name}</Link>
                                        <span className="text-sm text-gray-400 mb-2.5">Unity: R$ {product.price}</span>
                                        <h1>Quantity: {product.quantity}</h1>
                                    </div>
                                    <div className="font-semibold text-sm md:text-base text-heading leading-5">
